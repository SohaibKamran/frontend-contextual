...
export class TagSummaryComponent implements OnInit, OnDestroy {
      ...
      next: (v: any) => {
        ...
        for (let i = 0; i < retailerNames?.length; i++) {
          ...
          for (let j = 0; j < products.length; j++) {
            if(products[j].toDisplayAd==0) continue;
            if(products[j]?.minimumVzotScore)
            {
              minValue = products[j]?.minimumVzotScore
            }
            ...
            const color = products[j]?.Color?.name;
            ...
            productDetails.push({
              ...
              minimumVzotScore: products[j].minimumVzotScore,
              title: title,
              brand: brand,
          ...
          let retailer_obj = {
            id: retailerId,
            imageUrl: this.tagSummary?.Retailers[retailerNames[i]].imageUrl,
            name: this.tagSummary?.Retailers[retailerNames[i]].name,
            selected: true
          }
          this.alreadySelectedFilterArray.push(retailer_obj);
        ...
        for (let i = 0; i < retailerNames.length; i++) {
          ...
          for (let j = 0; j < products.length; j++) {
            ...
            if (products[j].superProxy) {
              this.superProxyImage =products[j]?.matchedImageUrl;
  ...
  openAddToCartModal(product) {
    ...
    activeModal.componentInstance.proxyCoordinateId = product?.proxyCoordinateId//product?.CoordinateHasProducts[0]?.ProxyCoordinateIdAdded?product?.CoordinateHasProducts[0]?.ProxyCoordinateIdAdded:product?.CoordinateHasProducts[0]?.proxyCoordinateId
  ...
  private apiBody() {
    ...
    {
      if(this.retailerProductImages[x].hideRetailer==true) continue;
      for(let y in this.retailerProductImages[x]?.productDetails)
      {
        if(this.retailerProductImages[x]?.productDetails[y]?.toDisplayAd)
        {
          filteredProducts.push(this.retailerProductImages[x]?.productDetails[y])
        }
      }
    }
    ...
    filteredProducts.map(product=>{
      product.productTag = this.tagNames;
      product.productId = product.id;
      if(!product.retailerId) product.retailerId = (this.retailerArray.find(retailer => retailer.name === product.retailerName) || {}).id;
      product.vzotScore = product.score;
      delete product.score;
      delete product.image;
      delete product.id;
      delete product.retailerImage;
      delete product.retailerName;
      delete product.sceneId;
      delete product.title;
      delete product.description;
      delete product.sceneId;
      if(product.attributes) delete product.attributes;
    })
    if(this.superProxyProduct !== null)
    {
      const prod = this.productBody.find(p => p.superProxy==true && p.toDisplayAd==false)
      if(prod)
      {
        filteredProducts.unshift(prod);
      }
      if(!prod)
      {
        let superProxyObj = this.productBody.find(p => p.superProxy==true && p.toDisplayAd==true)
        if(superProxyObj)
        {
          superProxyObj.toDisplayAd = false;
          filteredProducts.unshift(superProxyObj);
        }
        else
        {
          filteredProducts.unshift(this.superProxyProduct);
        }
      }
    }
    for(let x in filteredProducts)
    {
      if(filteredProducts[x].superProxy==true)
      {
        for(let y in filteredProducts)
        {
          if(filteredProducts[y].superProxy==true && y!=x)
          {
            filteredProducts.map(item=>{
              if(item.superProxy)
              {
                item.toDisplayAd = item.toDisplayAd==0?1:true;
              }
            })
            return {
              calledFromPrModal: true,
              id: this.idForImage,
    ...
    this.retailerProductImages.map(item=>{
      if(item.productDetails?.length>0)
      {
        item.productDetails.map(item2=>{
          if( item2.minimumVzotScore == undefined) item2.minimumVzotScore = item?.minimumVzotScore ? item?.minimumVzotScore : 0;
  ...
  ChangeVzotScore(product: any) {
    ...
    let boddy = {
      ...
      limit:100,
      proxyCoordinateId: this.data?.id? this.data?.id : this.dynamicId,

    }
    this.ngxService.startLoader("loader-01");
    this.apiService.sendRequest(requests.getProductsOfCoordinate, 'post', boddy).subscribe((data:any)=>{
      console.log(data);
  ...
  getOptimizeAdsProducts(filter?: any, loader=true) {
    ...
    if (this.superProxyImage) {
      const body = {
        ...
        genderId: this.selectedGender ?this.selectedGender:undefined ,
      };
      this.apiService.sendRequest(requests.getProductsOfCoordinate, 'post', body).subscribe((data: any) => {
        let v = data;
        this.tagSummary.Retailers = data?.data?.rows;
        this.retailerProductImages = []
        this.proxyImage = this.tagSummary.proxyImageUrl;
        sessionStorage.setItem('droppedImageUrl', this.proxyImage)
        this.sceneImgURL = this.tagSummary.Scene.thumbnail;
        ...
        this.sceneProductsData = v?.data?.rows;
        const retailerNames = Object.keys(this.sceneProductsData);
        for (let i = 0; i < retailerNames?.length; i++) {
          let retailerId;
          let products = this.tagSummary.Retailers[retailerNames[i]]?.Products;
          if(!products) products = this.tagSummary.Retailers[retailerNames[i]];
          if(!products) continue;
          const productDetails = [];
          let minValue = 0;
          for (let j = 0; j < products?.length; j++) {
            if(products[j]?.minimumVzotScore && products[j].toDisplayAd == 1)
            {
              minValue = products[j]?.minimumVzotScore
            }
            retailerId = products[j].Retailer?.id;
            const productId = products[j].id;
            const colorId = products[j]?.Color?.id ? products[j]?.Color?.id : products[j]?.Colors?.id
            const productImages = products[j]?.ProductImage ? products[j]?.ProductImage : products[j]?.Colors?.ProductImage;
            const productImageId = productImages?.id;
            const score = products[j]?.vzotScore;
            const matchedImageUrl = products[j].matchedImageUrl;
            const image = productImages?.reuploadedImages?.images[0];
            const id = products[j].id;
            const retailerImage = products[j]?.Retailer?.imageUrl// this.tagSummary?.Retailers[retailerNames[i]]?.imageUrl;
            const retailerName = products[j]?.Retailer?.name // this.tagSummary?.Retailers[retailerNames[i]]?.name;
            const superProxy = products[j].superProxy?true:false;
            const title = products[j]?.title;
            const brand = products[j]?.Brand?.name;
            const description = products[j]?.description;
            const color = products[j]?.Color?.name ?  products[j]?.Color?.name : products[j].Colors?.name;
            productDetails.push({
              score: parseInt(score),
              image: image,
              id: id,
          ...
          if (!filter) {
            ...
            if(!this.retailerProductImages.includes(retailerObject))
            this.retailerProductImages.push(retailerObject);
            this.retailerArray.push(retailerObject);
          }
          else {
        ...
        for (let i = 0; i < retailerNames.length; i++) {
          let tempImages = [];
          const products = this.sceneProductsData[retailerNames[i]].Products ? this.sceneProductsData[retailerNames[i]].Products: this.sceneProductsData[retailerNames[i]];
          for (let j = 0; j < products?.length; j++) {
            tempImages = [];
            if (products[j].superProxy) {
              this.superProxyImage =products[j]?.matchedImageUrl;

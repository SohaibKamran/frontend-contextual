...
export class StreamerTableComponent {
  ...
  selectStreamer(index){
    if(this.selectedStreamerIndex === index){
      this.selectedStreamerIndex = null;
    }else{
      this.selectTab(this.tabOption)
      this.selectedStreamerIndex = index;
    }
  }
  scrollToElement() {
    setTimeout(() => {
  ...
  fetchStreamers(series:any = null){
    this.body={
      ...this.body,
      pageNo:this.pagination.pageNo,
      limit:this.pagination.limit
    ...
    this.loader1 = true
    this.apiService.sendRequest(requests.getStreamersWithShows, 'post', this.body)
      .subscribe((res: any) => {
        this.loader1 = false
        if (res?.data?.rows){
        ...
        this.streamersTotalCount = res?.data?.count
        for (let index = 0; index < this.streamers.length; index++) {
          for (let i = 0; i < this.streamers[index].Shows?.length; i++) {
            if (this.streamers[index].Shows[i].videoTypeId == 3) {
              this.assignedMovieTaggers(i)
            }
            this.streamers[index].Shows[i].icon = "bi bi-chevron-right"
            this.streamers[index].Shows[i].clicked = false
            if (series) {
              if (series.id === this.streamers[index].Shows[i].id) {
                sessionStorage.removeItem('series')
            ...
            if (this.streamers[index].Shows[i]?.showStatusId == 1) {
              this.streamers[index].Shows[i].checked = true
            }
            else {
              this.streamers[index].Shows[i].checked = false
            }
          }
        }
      },(error)=>{
  ...
  tabOption = "";
  selectTab(option){
    if(this.tabOption === option)
      this.tabOption = null;
    else{
      this.tabOption = option;
      this.selectStreamer(this.selectedStreamerIndex)
    }
  }

  openModal(id?: number, seriesId?: number) {
    if (id != 3) {
      const modal = this.modalService.open(StreamerDetailsComponent, {
  ...
  updatePRTaggingStatus($event, sceneId: number) {
    ...
    this.apiService.sendRequest(requests.updateTagProductInScene, 'post', body)
      .subscribe((res) => {
        ...
        modalRef.componentInstance.editOrNew = 'new'
        modalRef.componentInstance.sceneId = this.sceneId
        // console.log(res)
        this.toastrService.success("Status updated successfully");

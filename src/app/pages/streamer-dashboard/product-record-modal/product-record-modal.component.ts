...
export class ProductRecordModalComponent implements OnInit, OnDestroy {
  selectedScreen = 1;
  isClipOn = false;
  inputEmbedding = [];
  ...
  NextScreen() {
    ...
    if (this.selectedScreen == 3 && !this.fetch_actors)
      this.fetchActors();
    else if (this.selectedScreen == 4 && this.superProxyImage) {
  ...
  open(content?: any) {
    ...
    modalRef.componentInstance.passEntry.subscribe((receivedEntry) => {
      if (receivedEntry) {
        ...
        if (this.alreadySelectedFilterArray && this.alreadySelectedFilterArray.length > 0)
          this.advertiserFilterProducts.forEach(retailer => {
            this.selectedRetailers.forEach(advertiser => {
  ...
  private apiBody() {
    ...
    // console.log(this.eliminatedProducts);
    // console.log(this.superProxyProduct);
    ...
    if (this.superProxyProduct !== null) {
      ...
      else //if(this.superProxyProduct.toDisplayAd==true || this.superProxyProduct.toDisplayAd==1)

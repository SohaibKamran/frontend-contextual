...
@Component({
  selector: 'app-streamer-records',
  templateUrl: './streamer-records.component.html',
  styleUrls: ['./streamer-records.component.scss']
})
export class StreamerRecordsComponent {
  users = []
  advertisers = [];
  streamers = [];
  ...
  constructor(
    ...
    private streamerService: ManageStreamerService,
  ...
  setChild(index: any, id) {
    ...
    this.streamerService.advertiserId = id;
  }
  ...
  selectStreamer(index){
    if(this.selectedStreamerIndex === index){
      this.setChild(this.selectedIndex,this.activeChildId)
  ...
  fetchStreamers(){
    ...
    this.loader1 = true
    this.selectedItems = []
    this.apiService.sendRequest(requests.getStreamersWithAdvertisers, 'post', this.body)
      .subscribe((res: any) => {
        this.loader1 = false
        this.streamers = res?.data?.ads?.rows;
        this.streamersTotalCount = res?.data?.ads?.count
      },(error)=>{
        this.streamers = []
      })
  }
